...
float4 VolumetricFog(float2 uv, float2 positionCS)
{
    ...
    // Jitter seed forced to 0 (was _FrameCount) so the raymarch dither is STATIC in screen space instead
    // of animating every frame. The animated jitter is meant to be resolved by temporal reprojection/TAA,
    // which this package doesn't do — so without it the low-res cone visibly CRAWLS. A static pattern kills
    // the crawl; the faint fixed dither it leaves is hidden by BuckshotPost's own dither + posterize floor.
    float jitter = stepLength * InterleavedGradientNoise(positionCS, 0);
